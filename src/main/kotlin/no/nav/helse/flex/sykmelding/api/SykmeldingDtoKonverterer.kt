    ...
    internal fun konverterSykmeldingSporsmal(sporsmal: List<Sporsmal>): SykmeldingSporsmalSvarDto {
        ...
        fun <T> Sporsmal.tilSvarListe(mapper: (verdi: String) -> T): FormSporsmalSvar<List<T>>? =
            if (svar.isEmpty()) {
                ...
                FormSporsmalSvar(sporsmaltekst = this.sporsmalstekst ?: "", svar = svar.map { mapper(it.verdi) })
            }
        ...
        return SykmeldingSporsmalSvarDto(
            ...
            uriktigeOpplysninger =
                hentSporsmal(sporsmal, SporsmalTag.URIKTIGE_OPPLYSNINGER)?.tilSvarListe { enumValueOf<UriktigeOpplysningerType>(it) },
            arbeidssituasjon =
            ...
            egenmeldingsdager = hentSporsmal(sporsmal, SporsmalTag.EGENMELINGSDAGER)?.tilSvarListe { LocalDate.parse(it) },
            ...
            fisker =
                hentSporsmal(sporsmal, SporsmalTag.FISKER)?.let { sp ->
                    FiskerSvar(
                        ...
                        lottOgHyre =
                            hentSporsmal(sp.undersporsmal, SporsmalTag.FISKER__LOTT_OG_HYRE)?.tilSvar { enumValueOf(it) }
